...
export function uploadedFileItem(doc, parentElement) {
  ...
  <div class="download_component" id=${doc.upload_id}>
    ...
    <div class="download_file-size hide-mobile-landscape">
      ...
      <div w-el="document_uploaded_fileSize" class="text-size-small hide-mobile-landscape">${returnFileSize(file.size)}</div>
    </div>
...
export async function renderDocuments(submodule_id) {
  try {
    ...
    if (submodule_id === null) {
      ...
      para.textContent = "Currently, we only offer tailored support for the cities of Berlin and Munich. If you are considering relocating to a different city in Germany, please note that specific assistance might be limited. We're constantly working to expand our services to more locations. Thank you for your understanding!";
      ...
      // Filter the documents based on their upload status
      const uploadedDocuments = docs.filter(doc => doc.document_uploaded === 1);
      const notUploadedDocuments = docs.filter(doc => doc.document_uploaded === 0);

      // Select the parent elements for document rendering
      const parentElementMissing = document.querySelector('[w-el="document_missing_list"]');
      const parentElementUploaded = document.querySelector('[w-el="document_uploaded_list"]');

      // Remove loader elements if present
      handleLoaderRemoval(parentElementMissing, '.loader_form');
      handleLoaderRemoval(parentElementUploaded, '.loader_form');
...
export async function handleUploadedDocuments() {
  const uploadedDocumentsList = document.querySelector('[w-el="document_uploaded_list"]');

  uploadedDocumentsList.addEventListener("click", async function (event) {
...
async function handleDownloadAction(parentLink) {
  ...
  const filename = toSafeFileName(documentTitle) + "." + attachmentUrl.split(".").pop();
...
function handleDeleteToggle(parentLink) {
  const card = parentLink.closest('.download_component');
  const deleteReminder = card.querySelector('[w-el="document_uploaded_deleteReminder"]');
...
export function hideFilePreview(event) {
  ...
  for (let level = 0; targetElement != null && level < maxLevels; level++) {
      if (targetElement.matches('[w-el="upload_closeButton"]')) {
          ...
          const previewFileWrapper = card.querySelector('[w-el="upload_previewFileWrapper"]');
